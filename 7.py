...
class ScreenshotTool(QMainWindow):
    ...
    def load_saved_screenshots(self):
        ...
        for filename in os.listdir("screenshots"):
            if filename.endswith(".png"):
                ...
                self.screenshots.append((filename, creation_time))

        # 按创建时间倒序排序
        self.screenshots.sort(key=lambda x: x[1], reverse=True)

        for filename, creation_time in self.screenshots:
            time_str = datetime.fromtimestamp(creation_time).strftime("%Y-%m-%d %H:%M:%S")
            self.screenshot_list.addItem(f"{time_str} - {filename}")
    ...
    def capture_screenshot(self):
        ...
        save_dialog = SaveDialog(screenshot, self)
        if save_dialog.exec_():
            screenshot.save(os.path.join("screenshots", save_dialog.filename))
            self.load_saved_screenshots()
